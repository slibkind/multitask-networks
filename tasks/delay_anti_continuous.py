...
class DelayAntiContinuous(Task):
        ...
        for i in range(batch_size):
            ...
            for period in range(self.num_periods):
                if period == 0:  # fixation period
                    ...
                    inputs[i, period*period_duration:(period+1)*period_duration, 0] = 1
                    outputs[i, period*period_duration:(period+1)*period_duration, 1] = np.sin(stimulus + np.pi)
                    outputs[i, period*period_duration:(period+1)*period_duration, 2] = np.cos(stimulus + np.pi)
        ...
        for s, stimulus in enumerate(self.stimuli):
            for period in range(self.num_periods):
                if period == 0:  # fixation period
                    ...
                    inputs[s, period*period_duration:(period+1)*period_duration, 0] = 1
                    outputs[s, period*period_duration:(period+1)*period_duration, 1] = np.sin(stimulus + np.pi)
                    outputs[s, period*period_duration:(period+1)*period_duration, 2] = np.cos(stimulus + np.pi)
