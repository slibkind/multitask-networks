...
def plot_models(model_list, q_thresh_list, params_list, n_interp, save_path):
    ...
    for (model_name, epoch) in model_list:
        ...
        fig.suptitle(f'Model: {model_name}, Epoch: {epoch}', fontsize=16)  # Set the title for the whole plot
        ...
        plt.tight_layout()
        plt.subplots_adjust(top=0.90)  # Adjust the subplot to make room for the suptitle
...
params_list = [
    ...
    },
    {
        ...
        'period': ["delay", "delay"],
        'stimulus': [1, 1],
        'labels': ["DelayGo - delay", "DelayAnti - delay"],
        'title': f"Comparing the delay periods",
    }
]

n_interp=10
q_thresh = [1e-2, 1e-4, 1e-6]
model_name = "delaygo_delayanti_256"

model_list = [(model_name, epoch) for epoch in range(2000, 18000, 2000)]

save_path = "/Users/slibkind/Desktop/figures2"

      
plot_models(model_list, q_thresh, params_list, n_interp, save_path)  
