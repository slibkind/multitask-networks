...
def get_model_checkpoint(model_name, epoch=None):
    model_path = get_model_path(model_name, epoch=epoch)
    return torch.load(model_path)

def load_checkpoint(model_name, model, optimizer, epoch=None):
    """Load a model and optimizer state from a checkpoint file."""
    checkpoint = get_model_checkpoint(model_name, epoch=epoch)
...
def get_tasks(model_name, epoch=None):
    model_checkpoint = get_model_checkpoint(model_name, epoch=epoch)
    return model_checkpoint['tasks']

def get_model(model_name, epoch=None):
    model_checkpoint = get_model_checkpoint(model_name, epoch=epoch)
...
def get_model_epoch(model_name, epoch = None): 
    checkpoint = get_model_checkpoint(model_name, epoch=epoch)
